...
struct DivisoresTrade //user defined datatype
{
	int arcilla;
	int trigo;
	int oveja;
...
// Problem ----------------------------------------------------------------------------
  ...
  requires class Problem
  {
	public:
		...
		int dimension() const;
		char terrenos(int i) const;
		int numeros(int i) const;
		char puertos(int i) const;
		bool construccionesOponentes(int i) const;
		bool aristasOcupadas(int i, int j) const;
		int construcciones(int i) const;
		bool caminos(int i, int j) const;
		Reserva reserva() const;
		int dados(int i, int j) const;
		bool verticesValidos(int i) const;

	private:

		int _dimension; // calculado
		char* _terrenos;//[19];
		int* _numeros;//[19];
		char* _puertos;//[9];
		bool* _construccionesOponentes;//[54];
		bool** _aristasOcupadas;//[54][54]; // estructura magica de dijkstra
		int* _construcciones;//[54];
		bool** _caminos;//[54][54]; // estructura magica de dijkstra
		Reserva _reserva;
		int** _dados; //[cantidadSimulaciones][largoMaximoSimulacion]
		bool* _verticesValidos; //[54]
...
//Solution ----------------------------------------------------------------------------
  ...
  requires class Solution
  {
	public:
		...
		void corregirSolucion();
		void initialize();
		//void initializeGreedy();
		void mapearRecursos (Reserva* &mapeo);
		bool alcanzanRecursos(Reserva &tengo, Reserva quiero, DivisoresTrade divisores);
		void cobrarRecursos(int dadoActual, Reserva &recursos, int* construcciones);
		int correrSimulacion(int idxSimulacion, Reserva* mapeo);
		double fitness ();
